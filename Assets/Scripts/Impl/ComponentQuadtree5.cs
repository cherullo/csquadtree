...
//            return;
    ...
    protected void Rebuild(ComponentQuadtree5<T> root)
    {
        if (m_childCount == 0)
        {
...
//                Debug.Log ("Back to bucket mode");
                ...
                m_nodes [0].Rebuild (root);
                m_nodes [1].Rebuild (root);
                m_nodes [2].Rebuild (root);
                m_nodes [3].Rebuild (root);
            }

            return;
        }

        if (m_childCount == root.m_childCount)
        {
//            Debug.Log ("Nice " + m_childCount);
            root.m_firstDecisionPoint = this;
        }
